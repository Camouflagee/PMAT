...
class SharedReplayBuffer(object):
    ...
    def __init__(self, args, num_agents, obs_space, cent_obs_space, act_space, env_name):
        ...
        self.action_log_probs = np.zeros(
            (self.episode_length, self.n_rollout_threads, num_agents, act_shape), dtype=np.float32)
        self.seq_log_probs = np.zeros(
            (self.episode_length, self.n_rollout_threads, 1, 1), dtype=np.float32
        )
        self.seqs = np.zeros(
            (self.episode_length, self.n_rollout_threads, 1, num_agents), dtype=np.float32)
    ...
    def insert(self, share_obs, obs, rnn_states_actor, rnn_states_critic, actions, action_log_probs, seq_log_probs, seqs,
        ...
        self.seq_log_probs[self.step] = seq_log_probs.copy()
        self.seqs[self.step] = seqs.copy()
    ...
    def feed_forward_generator_transformer(self, advantages, num_mini_batch=None, mini_batch_size=None):
        ...
        seq_log_probs = self.seq_log_probs.reshape(-1, *self.seq_log_probs.shape[2:])
        seqs = self.seqs.reshape(-1, *self.seqs.shape[2:])
        ...
        for indices in sampler:
            ...
            seq_log_probs_batch = seq_log_probs[indices].reshape(-1, *seq_log_probs.shape[2:])
            seqs_batch = seqs[indices].reshape(-1, *seqs.shape[2:])
            ...
            yield share_obs_batch, obs_batch, rnn_states_batch, rnn_states_critic_batch, actions_batch, \
                  ...
                  seq_log_probs_batch, seqs_batch, adv_targ, available_actions_batch
